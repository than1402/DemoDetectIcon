...
BOOL CPictureCtrl::LoadFromFile(CString &szFilePath)
{
	...
	//pDC->StretchBlt(0, 0, client_rect.Width(), client_rect.Height(), &dc, 0, 0,
	//	image.GetWidth(), image.GetHeight(), SRCCOPY);
	//dc.DeleteDC();
	//pDC->DeleteDC();
	//this->ReleaseDC(pDC);
	//image.Destroy();
...
BOOL CPictureCtrl::Load(CString &szFilePath, CRect &rect)
{
	return LoadFromFileRect(szFilePath, rect);
}

BOOL CPictureCtrl::LoadFromFileRect(CString &szFilePath, CRect &rect)
{
	CDC *pDC, dc;
	CRect client_rect;
	pDC = this->GetDC();
	...
	dc.CreateCompatibleDC(pDC);
	dc.SelectObject(image);
	//pDC->BitBlt(0, 0, client_rect.Width(), client_rect.Width(), &dc, 0, 0, SRCCOPY);
	pDC->StretchBlt(0, 0, client_rect.Width(), client_rect.Height(), &dc, rect.left, rect.top,
